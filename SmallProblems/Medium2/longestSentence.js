...
/* 
  ...
  input: string--several sentences
  output: longest sentence string

  rules:
  1. Words can contain any characters
  2. Sentences end with ! . or ?
  3. Any sequence of chars is a word
  4. Preserve punctuation from the sentence

  Algo:
  1. Separate sentences from the text into an array of sentences
  2. Separate each sentence into an array of words
  3. count the words in each sentence
  4. somehow keep track of the num of words and which snetence it is
  5. return the longest snetence
...
function longestSentence(text) {
  ...
  sentences.forEach((sentence) => {
    let senLength = sentence.split(' ').length;
    if (senLength > howLong) {
      howLong = senLength;
      longestOne = sentence.join(' ');
    }
  });

  return `${longestOne} \n \nThe longest sentence has ${howLong} words.`;
}
